    ...
    with torch.no_grad():
        for step in tqdm(range(num_steps - 1)):
            ...
            if curr_best_terminal_logG != float('-inf') and best_terminal_logG <= curr_best_terminal_logG:
                ...
                best_terminal_smiles_seq = tokenizer.batch_decode(best_terminal, skip_special_tokens=True)[0].replace(' ', '')
                print("\nSaved Best Terminal: ", best_terminal_smiles_seq)
                print("Saved Best Terminal Length: ", best_terminal.shape[1])
...
def main():
    ...
    parser.add_argument("--root_dir", type=str, default="/scratch/pranamlab/tong/pCoMol/gfp/FPredX")
    ...
    parser.add_argument("--output_file", type=str, default='sample.csv')
    ...
    file_name = args.output_file.split('/')[-1].split('.')[0]
    if file_name == 'length_excitation_brightness':
        objective_models = [length, excitation, brightness]
        # args.objective_weights = [3,1,1]
    elif file_name == 'length_brightness':
        objective_models = [length, brightness]
        # args.objective_weights = [3,1]
    elif file_name == 'length_excitation':
        objective_models = [length, excitation]
        # args.objective_weights = [3,1]
    elif file_name == 'length':
        objective_models = [length]
        # args.objective_weights = [1]
    ...
    compute_scores_print([args.input], objective_models, constraint_models, device, return_scores=True)
    return
    # pdb.set_trace()
    for _ in range(100):
        x_T = cope_strict(
        ...
        print("Final scores:")
        scores = compute_scores_print([out_str], objective_models, constraint_models, device, return_scores=True)

        with open(args.output_file, 'a') as f:
            f.write(f"{out_str},{len(out_str)}")
            for score in scores:
                f.write(f",{score}")
            f.write('\n')
