...
from classifier_prediction import GFPClassifier, load_model_from_ckpt
...
class GFPEmissionPred:
    """
    Predict GFP emission maximum (em_model) using the FPredX pipeline:
    ...
    def get_score(
        ...
        for pred in mean_pred:
            scores.append(1 if float(pred) >= 470 and float(pred) <= 550 else 0)
        return scores
...
class Length:
    ...
    def __call__(self, seqs):
        # return [1 if len(seq) < len(self.orig_seq) and len(self.orig_seq) - len(seq) <= 25 else 0 for seq in seqs]
        return [1 if len(seq) == 213 else 0 for seq in seqs]
...
class GFP:
    ...
    def __call__(self, seq):
        return self.get_scores(seq)
    
    def get_scores(self, seqs, return_probs=False):
