...
def process_latent_batch(smiles, protein_seqs):
    data_list = []
    for smile, protein_seq in zip(smiles, protein_seqs):
        smile_graph = smile_to_graph(smile)
        XT = seq_cat(protein_seq)

        c_size, features, edge_index, edge_feats = smile_graph
        GCNData = DATA.Data(x=torch.Tensor(np.array(features)),
                            edge_index=torch.LongTensor(edge_index).transpose(1, 0),
                            edge_attr=torch.Tensor(np.array(edge_feats)))
        GCNData.target = torch.from_numpy(XT).long().unsqueeze(0)
...
def process_latent_a(smile, protein_seq):
    ...
    GCNData = DATA.Data(x=torch.Tensor(np.array(features)),
                        edge_index=torch.LongTensor(edge_index).transpose(1, 0),
                        edge_attr=torch.Tensor(np.array(edge_feats)))
    GCNData.target = torch.from_numpy(XT).long()
    GCNData.__setitem__('c_size', torch.LongTensor([c_size]))   
